...
export const BranchHeader: React.FC<BranchHeaderProps> = ({
  ...
  const [editingUrls, setEditingUrls] = useState<Record<Branch, string>>({
    Dondaicha: configs.find(c => c.id === 'Dondaicha')?.webAppUrl || '',
  ...
  // Keep local editing state perfectly in sync with parent configs prop
  React.useEffect(() => {
    setEditingUrls({
      Dondaicha: configs.find(c => c.id === 'Dondaicha')?.webAppUrl || '',
      Hojiwala: configs.find(c => c.id === 'Hojiwala')?.webAppUrl || '',
    ...
    <div id="branch-header" className="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center bg-gray-50 p-1.5 rounded-xl border border-gray-100">
            {(['Dondaicha', 'Hojiwala', 'Sachin', 'Ambernath'] as Branch[]).map((branchName) => {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-10 text-center">
            {/* Backdrop */}
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity animate-fade-in z-40" onClick={() => setIsOpen(false)}></div>

            {/* Modal Card */}
            <div className="relative z-50 bg-white rounded-2xl text-left shadow-2xl transform transition-all w-full max-w-2xl max-h-[85vh] flex flex-col border border-slate-200 overflow-hidden animate-zoom-in my-8">
              {/* Header */}
              <div className="bg-slate-50 px-6 py-4 border-b border-gray-150 flex items-center justify-between shrink-0">
                <div className="flex items-center space-x-2.5">
                  <div className="bg-emerald-50 p-2 rounded-lg">
                    <Settings className="w-5 h-5 text-emerald-600" />
                  </div>
                  <h3 className="text-lg font-extrabold text-slate-800" id="modal-title">
              ...
              {/* Scrollable Content */}
              <div className="px-6 py-5 overflow-y-auto space-y-5 flex-1 min-h-0">
                <div className="text-sm text-gray-600 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100/50 leading-relaxed">
                  ...
                  <p className="mt-1 text-xs">
                    This form writes directly to your target Google Spreadsheet (ID: <code className="bg-white px-1 py-0.5 rounded border text-emerald-750 font-mono text-xs">{configs.find(c => c.id === currentBranch)?.spreadsheetId || '107lQoWsOCPpSNFm4L36Ym2EFElxsdgeEPMfGYvBCj5M'}</code>) via a secure Google Apps Script Web App. Simply deploy the script below in your Spreadsheet.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {(['Dondaicha', 'Hojiwala', 'Sachin', 'Ambernath'] as Branch[]).map((branch) => (
                    <div key={branch} className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-gray-700 flex items-center justify-between">
                        <span className="text-slate-700 font-bold">{branch} Branch API Endpoint</span>
                        <span className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded font-mono font-extrabold ${editingUrls[branch] ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-150 text-amber-800'}`}>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 bg-slate-50 hover:bg-slate-100/50 focus:bg-white text-slate-900 border-2 border-slate-200 rounded-lg text-xs sm:text-sm font-mono placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium"
                      />
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-gray-55 hover:bg-gray-100 text-slate-750 border border-gray-200 rounded-lg text-xs font-semibold flex items-center space-x-1 transition-all"
                    >
                  ...
                  <ol className="text-xs text-gray-600 list-decimal pl-4 space-y-1.5 font-medium">
                    <li>Open target spreadsheet <a href={`https://docs.google.com/spreadsheets/d/${configs.find(c => c.id === currentBranch)?.spreadsheetId}/edit`} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline inline-flex items-center gap-0.5 font-bold">Open Sheet <ExternalLink className="w-2.5 h-2.5" /></a></li>
                  ...
                  <div className="relative">
                    <pre className="text-[10px] font-mono select-all bg-slate-900 text-slate-100 p-3.5 rounded-xl max-h-40 overflow-y-auto leading-relaxed border border-slate-800">
              ...
              {/* Footer */}
              <div className="bg-slate-50 px-6 py-4 border-t border-gray-150 flex items-center justify-end gap-3 shrink-0">
                <button
                  onClick={() => setIsOpen(false)}
