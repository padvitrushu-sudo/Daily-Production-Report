...
export const BranchHeader: React.FC<BranchHeaderProps> = ({
  ...
  return (
    <div id="branch-header" className="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm transition-all">
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          {/* Backdrop */}
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity animate-fade-in" onClick={() => setIsOpen(false)}></div>

          {/* Modal Card */}
          <div className="relative z-10 bg-white rounded-2xl text-left shadow-2xl transform transition-all w-full max-w-2xl max-h-[85vh] flex flex-col border border-slate-200 overflow-hidden animate-zoom-in">
            {/* Header */}
